...
## tsconfig 配置规范
  ...
  > 使用 `tsc --init`自动生成配置文件，并根据如下推荐做修改。
...
## 文件夹及文件
  ...
  - [2.1](#2.1) <a name="2.1"></a> **文件夹**一律使用`小写`命名。使用 `"-"` 来分隔描述性单词。例如 `react-dom` , `vue-router`。
  ...
  - [2.5](#2.5) <a name="2.5"></a> `export name` 应尽量与 `filename` 保持一致。使用命名导出使而不是默认导出。
...
## 总体命名原则
  ...
  - [3.1](#3.1) <a name="3.1"></a> 命名原则上一律使用英文单词，原则上不建议使用拼音，要见名知意，不常见的单词需添加中文注释。
  ...
  - [3.6](#3.6) <a name="3.6"></a> 原则上任何命名不允许缩写。如下几种情况可适当考虑缩写。如有缩写的情况下，宜采用缩写单词前面几个字母或可以音译的为佳，并应配有相应的注释。

    ```typescript
    // object 为关键字
    const obj = {};
    // 单词较长且约定俗称，且缩写不会被拼写检查插件报错。
    ...
    let message, error
    ```
  - [3.7](#3.7) <a name="3.7"></a> 若无特殊说明，默认使用 `camelCase`，保留前置下划线，但不是用于表达私有变量，多用于不可变数据重新赋值和函数中未使用变量的占位，考虑添加 `$` 来表示一类特殊的值，例如 *RxJS* 中的 *Observable*。
...
## 符号
  ...
  - [5.2](#5.2) <a name="5.2"></a> 使用单引号，jsx 中使用双引号。
...
## 变量
  ...
  - [6.2](#6.2) <a name="6.2"></a> 变量使用 `camelCase` 命名，常量可使用 `camelCase, UPPER_CASE, PascalCase` 命名。
    ...
    let a = 1;
    // 全局的常量使用 UPPER_CASE
    const API_URL = 'http://www.baidu.com.cn/api';
    function fn() {
      // 块内部的常量使用 camelCase
      const defaultValue = 1;
    }
    // 当常量表示一个组件时，使用 PascalCase
...
## 数组
  ...
  - [8.3](#8.3) <a name="8.3"></a> 向数组添加元素时使用 Array#push 替代直接赋值。
...
## 枚举
  ...
  - [10.1](#10.1) <a name="10.1"></a> 枚举使用 `PascalCase`，枚举值使用 `UPPER_CASE` 。
...
## 函数
  ...
  - [13.11](#13.11) <a name="13.11"></a> 函数（方法）及其参数总是使用 `camelCase`。
...
1. [JavaScript Standard Style](https://standardjs.com/index.html)
1. [Angular Style Guide](https://angular.io/docs/ts/latest/guide/style-guide.html)
